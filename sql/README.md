...
## Автоматичні резервні копії (11.08.2026)

### Навіщо

Основне сховище CRM — `localStorage` браузера. Чистка кешу,
переустановка системи чи зламаний телефон = втрата всього.
Синхронізація по таблицях рятує частково: у ній немає історії
і немає даних, які живуть лише локально (Б/У шини, ринкові ціни).

### Як працює

Таблиця `v4_backups`: `id` = дата (`YYYY-MM-DD`), `payload` = повний
зліпок JSON, `counts` = скільки чого.

CRM раз на день, **після чистої синхронізації**, кладе зліпок.
Повторний запис того ж дня оновлює наявний, не дублює.

Не пишемо, якщо: даних немає (щоб не затерти вчорашню робочу копію),
зліпок більше 4500 КБ, або синхронізація пройшла з помилками.

### Скільки зберігається

`cleanup_backups()` щодня о 09:05: усі копії за 14 днів + по одній
на місяць за рік. Решта видаляється.

### Якщо копій давно немає

`check_backup_freshness()` — якщо остання старша за 3 дні,
пише в Telegram. Не частіше разу на день.

### Відновлення

CRM → 🔒 top_secret → **Резервні копії** → натиснути дату.

Перед перезаписом автоматично завантажується файл із поточним
станом — щоб помилковий вибір дати не знищив те, що є зараз.

### Доступ

`anon` до `v4_backups` не має доступу взагалі — перевірено.

### Примітка про UNION

У першій версії `cleanup_backups()` була помилка: `ORDER BY`
у гілці `UNION` стосується всього обʼєднання, а не гілки.
`distinct on` треба загортати в підзапит.
